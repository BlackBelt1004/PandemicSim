...
class Person:
    ...
    # Super spreader - random movement 
...
class Neighborhood:
    ...
    def update_canvas_grid(self):
        ...
        self.canvas.update()
    def update_loop(self, max_symptomatic, total_people, susceptible, infected, recovered, dead, day_clock):
        ...
        if infected != 0:
            self.canvas.after(10, self.update_loop(max_symptomatic, total_people, susceptible, infected, recovered, dead, day_clock))
        elif infected == 0:
            recovered = 0
            dead = 0
            for row in range(HEIGHT):
            ...
            print("{:.2f}% of people recovered.".format((recovered/total_people) * 10))
            print("{:.2f}% of people died.".format((dead/total_people) * 10))
            print(" \n")
...
for density in range(10,100,10):
    for symptom_chance in range (10,100,10):
        ...
        POPULATION_DENSITY = density / 100
        n = Neighborhood()
        n.update_loop(max_symptomatic, total_people, susceptible, infected, recovered, dead, day_clock)
