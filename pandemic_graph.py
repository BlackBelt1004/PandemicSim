...
SCREEN_UPDATE_FREQUENCY = 10 # How often to redraw the screen.  Making this 5 or 10
                            # will cause the screen to redraw only every 5 or 10
                            # days, speeding up the run-time, slightly.

STARTING_INFECTED = 3       # How many "patient zero"s are there in your population?

STARTING_SPREADER = 1

POPULATION_DENSITY = .2    # Fraction of cells have a person in them

total_people = (WIDTH * HEIGHT) * POPULATION_DENSITY
...
class Person:
        ...
        for r in range(self.my_row - distance, self.my_row + distance + 1):
            for c in range(self.my_col - distance, self.my_col + distance + 1):
                ...
                if self.neighborhood.grid[r][c] != "empty" \
                        and self.neighborhood.grid[r][c].state in ["infected", "symptomatic", "spreader"]:
    ...
    def update(self):
        ...
        if self.state == "infected" or self.state == "symptomatic" or self.state == "spreader":
            self.days_left -= 1
        ...
        if self.state == "symptomatic" or self.state == "spreader":
...
class Neighborhood:
    def __init__(self):
        ...
        unlucky_person2 = random.choice(self.person_list)
        unlucky_person2.state = "spreader"
        unlucky_person2.days_left = INFECTION_LENGTH
    ...
    def update_loop(self, max_symptomatic, total_people, healthy_people, susceptible, infected, recovered, dead, day_total, POPULATION_DENSITY, day_clock):
        # update the clock
        self.clock += 1
        day_total += 1
        day_clock.append(day_total)
        self.master.title("Pandemic Day " + str(self.clock))
        ...
        for row in range(HEIGHT):
            for col in range(WIDTH):
                if self.grid[row][col] != "empty":
                    state = self.grid[row][col].state
                    if state in ["infected", "symptomatic"]: 
                        infected_today += 1
        ...
        if recovered[-1] > recovered[-2]:
            new_recovered = True
        elif recovered[-1] == recovered[-2]:
            new_recovered = False
        
        if new_recovered == True:    
            infected_today += (recovered[-1] - recovered[-2])
            if new_dead == True:
                infected_today += (dead[-1] - dead[-2])
                if infected_today > infected[-2]:
                    healthy_people -= infected_today - infected[-2]
                    susceptible.append(healthy_people)
                    # infected_total += infected_today - infected[-2]
                elif infected_today == infected[-2]:
                    healthy_people -= 0
                    susceptible.append(healthy_people)
            elif new_dead == False:
                if infected_today > infected[-2]:
        ...
        if symptomatic > max_symptomatic:
            max_symptomatic = symptomatic
        if infected_today != 0:
            self.canvas.after(10, self.update_loop(max_symptomatic, total_people, healthy_people, susceptible, infected, recovered, dead, day_total, POPULATION_DENSITY, day_clock))
        elif infected_today == 0:
            recovered_total = 0
            dead_total = 0
            infected_total = 0
            infected_total = total_people - healthy_people
            for row in range(HEIGHT):
                for col in range(WIDTH):
                    if self.grid[row][col] != "empty":
                        state = self.grid[row][col].state
                        ...
                        if state in ["dead"]:
                            dead_total += 1            
            self.master.destroy()
            print("The pandemic lasted for " + str(self.clock) + " days and there were a total of " + str(max_symptomatic) + " people that were symptomatic at its height.")
            print("{:.2f}% of infected people recovered.".format((recovered_total/infected_total) * 100))
            print("{:.2f}% of infected people died.".format((dead_total/infected_total) * 100))
            print(" \n")
            ...
            sird_plot(susceptible, infected, recovered, dead, day_total, POPULATION_DENSITY, total_people, day_clock)
...
def sird_plot(susceptible, infected, recovered, dead, day_total, POPULATION_DENSITY, total_people, day_clock):
    global WIDTH
    global HEIGHT
    
    susceptible_x = day_clock
    susceptible_y = susceptible
    
    infected_x = day_clock
    infected_y = infected
    
    recovered_x = day_clock
    recovered_y = recovered
    ...
    pylab.plot(susceptible_x, susceptible_y, "k-", label = "susceptible")
    pylab.plot(infected_x, infected_y, "r-", label = "infected")
    pylab.plot(recovered_x, recovered_y, "g-", label = "recovered")
    pylab.plot(dead_x, dead_y, "y-", label = "dead")
    
    pylab.xlim(0, day_total + 10)
    pylab.ylim(0, total_people + 50)
    pylab.xlabel("Days")
    pylab.ylabel("Persons")
    pylab.title("Pandemic Day " + str(day_total))
    pylab.legend()
...
def parameter_study():
    ...
    for density in range(10,100,10):
        for symptom_chance in range(10,100,10):
            SYMPTOM_CHANCE = symptom_chance / 100
            POPULATION_DENSITY_ps = density / 100
            print("The parameters were a density of " + str(POPULATION_DENSITY_ps) + " and a symptom chance of " + str(SYMPTOM_CHANCE) + ".")
            total_people_ps = (WIDTH * HEIGHT) * POPULATION_DENSITY_ps
            susceptible_ps = [total_people_ps]
            healthy_people_ps = total_people_ps
            infected_ps = [0]
            recovered_ps = [0]
            dead_ps = [0]
            day_clock_ps = [0]
            ...
            n = Neighborhood()
            n.update_loop(max_symptomatic, total_people_ps, healthy_people_ps, susceptible_ps, infected_ps, recovered_ps, dead_ps, day_total_ps, POPULATION_DENSITY_ps, day_clock_ps)
